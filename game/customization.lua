...
local function setCamera(key)
    -- Guard: if key is "current" but currentCamera was cleared (e.g. after exitPlayerCustomization),
    -- do nothing to avoid indexing constants.CAMERAS with nil.
    if key == "current" and not currentCamera then return end
    if not isCameraInterpolating then
        ...
        if cameraHandle then
            ...
            local myGen    = camGeneration
            local oldHandle = cameraHandle
            cameraHandle   = tmpCamera  -- update immediately so concurrent ops see correct handle

            CreateThread(function()
                repeat Wait(100)
                until camGeneration ~= myGen or (not IsCamInterpolating(tmpCamera) and IsCamActive(tmpCamera))
                if DoesCamExist(oldHandle) then DestroyCam(oldHandle, false) end
                if camGeneration == myGen then
...
-- ══════════════════════════════════════
-- ORBIT CAMERA SYSTEM (Bone-Based Focus)
-- ══════════════════════════════════════
local isOrbitActive = false
local camGeneration = 0  -- incremented each time orbit/camera state resets; invalidates stale threads
local curAngleH   = 0.0
local curAngleV   = 0.0
local curDist     = 2.2
local curFocusZ   = 0.2
...
local function getClosestBoneFocusZ(cursorX, cursorY)
    local ped = cache.ped
    if not ped or not DoesEntityExist(ped) then return 0.2 end
    local pedPos     = GetEntityCoords(ped)
    local bestDist   = math.huge
    local bestFocusZ = 0.2
    for _, boneId in ipairs(ORBIT_BONES) do
        local bonePos = GetPedBoneCoords(ped, boneId, 0.0, 0.0, 0.0)
        local onScreen, screenX, screenY = World3dToScreen2d(bonePos.x, bonePos.y, bonePos.z)
        if onScreen then
            local dist = math.abs(screenY - cursorY)
            if dist < bestDist then
                bestDist   = dist
                bestFocusZ = bonePos.z - pedPos.z
            end
        end
    end
    return bestFocusZ
...
local function calcOrbitCamCoords(ah, av, dist, fz)
    local ped     = cache.ped
    local radH    = math.rad(ah)
    local radV    = math.rad(av)
    local cosV    = math.cos(radV)
    local offX    = dist * cosV * math.sin(radH)
    local offY    = dist * cosV * math.cos(radH)
    local offZ    = dist * math.sin(radV)
    local pedPos  = GetEntityCoords(ped)
    local heading = math.rad(GetEntityHeading(ped))
    local rotX    = offX * math.cos(heading) - offY * math.sin(heading)
    local rotY    = offX * math.sin(heading) + offY * math.cos(heading)
    return pedPos.x + rotX, pedPos.y + rotY, pedPos.z + offZ + fz, pedPos.x, pedPos.y, pedPos.z + fz
end

local function startOrbitCamera(data)
    local focusZ = getClosestBoneFocusZ(data.cursorX or 0.5, data.cursorY or 0.5)
    if isOrbitActive then
        tgtFocusZ = focusZ
        return
    end
    isOrbitActive = true
    camGeneration = camGeneration + 1  -- cancel any stale setCamera/rotateCamera thread
    local myOrbitGen = camGeneration  -- capture so orbit thread can self-terminate on cancelOrbit
    tgtFocusZ = focusZ
    curFocusZ = focusZ

    local cx, cy, cz, lx, ly, lz = calcOrbitCamCoords(curAngleH, curAngleV, curDist, curFocusZ)
    local orbitCam = CreateCameraWithParams("DEFAULT_SCRIPTED_CAMERA", cx, cy, cz, 0.0, 0.0, 0.0, 49.0, false, 0)
    PointCamAtCoord(orbitCam, lx, ly, lz)
    if cameraHandle then
        SetCamActiveWithInterp(orbitCam, cameraHandle, 500, 1, 1)
        local oldHandle = cameraHandle
        cameraHandle = orbitCam
        CreateThread(function()
            Wait(550)
            if DoesCamExist(oldHandle) then DestroyCam(oldHandle, false) end
        end)
    ...
    CreateThread(function()
        -- Dual exit condition: isOrbitActive flag (set by cancelOrbit) OR generation mismatch
        -- (handles any edge case where isOrbitActive isn't cleared in time)
        while isOrbitActive and camGeneration == myOrbitGen do
            Wait(0)
            curAngleH = curAngleH + (tgtAngleH - curAngleH) * 0.15
            curAngleV = curAngleV + (tgtAngleV - curAngleV) * 0.15
            curDist   = curDist   + (tgtDist   - curDist)   * 0.15
            curFocusZ = curFocusZ + (tgtFocusZ - curFocusZ) * 0.08
            if cameraHandle and DoesCamExist(cameraHandle) then
                local x, y, z, lx2, ly2, lz2 = calcOrbitCamCoords(curAngleH, curAngleV, curDist, curFocusZ)
                SetCamCoord(cameraHandle, x, y, z)
                PointCamAtCoord(cameraHandle, lx2, ly2, lz2)
            end
...
client.startOrbitCamera = startOrbitCamera

local function updateOrbitData(data)
    if not isOrbitActive then return end
    if data.angleH   then tgtAngleH = data.angleH   end
    if data.angleV   then tgtAngleV = data.angleV   end
    if data.distance then tgtDist   = data.distance end
    if data.cursorX and data.cursorY and data.updateFocus then
        tgtFocusZ = getClosestBoneFocusZ(data.cursorX, data.cursorY)
    end
end
client.updateOrbitData = updateOrbitData

-- cancelOrbit: resets orbit state and invalidates any running camera threads.
-- Does NOT switch to a preset camera — callers must do that themselves.
local function cancelOrbit()
    camGeneration      = camGeneration + 1  -- invalidate stale threads
    isCameraInterpolating = false           -- allow the next setCamera/rotateCamera call
    if not isOrbitActive then return end
    isOrbitActive = false
    curAngleH, curAngleV, curDist, curFocusZ = 0.0, 0.0, 2.2, 0.2
    tgtAngleH, tgtAngleV, tgtDist, tgtFocusZ = 0.0, 0.0, 2.2, 0.2
end
client.cancelOrbit = cancelOrbit

local function exitOrbitCamera()
    cancelOrbit()
    setCamera("current")
end
client.exitOrbitCamera = exitOrbitCamera
...
function client.rotateCamera(direction)
    if not isCameraInterpolating then
        ...
        local myGen    = camGeneration
        local oldHandle = cameraHandle
        cameraHandle   = tmpCamera  -- update immediately
...
function client.exitPlayerCustomization(appearance)
    -- MUST be called first: stops the orbit update thread (which disables controls every frame)
    -- and clears isCameraInterpolating so no stale camera threads block future sessions.
    cancelOrbit()
